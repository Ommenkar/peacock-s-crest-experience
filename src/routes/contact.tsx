      ...
      <section className="py-24">
        <div className="container-luxe grid lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7">
            ...
            <h2 className="mt-4 font-display text-4xl md:text-5xl leading-tight">Send us a note.</h2>
              ...
              >
                {[
                  ...
                  <label key={f.n} className="block">
                    <span className="text-[10px] tracking-[0.3em] uppercase text-muted-foreground">{f.l}</span>
                    <input required type={f.t} name={f.n} maxLength={120} className="mt-2 w-full bg-transparent border-b border-border focus:border-gold outline-none py-3 text-foreground" />
                  </label>
                ))}
                <label className="block sm:col-span-2">
                  <span className="text-[10px] tracking-[0.3em] uppercase text-muted-foreground">Message</span>
                  <textarea required name="message" maxLength={1000} rows={4} className="mt-2 w-full bg-transparent border-b border-border focus:border-gold outline-none py-3 text-foreground resize-none" />
                </label>
                <button type="submit" className="sm:col-span-2 justify-self-start mt-4 px-9 py-4 bg-primary text-primary-foreground text-xs tracking-[0.3em] uppercase hover:bg-forest-deep transition-colors">
                  Send Enquiry
                </button>
              </form>
            )}
          ...
          <aside className="lg:col-span-5 space-y-10">
            ...
            <div>
              ...
              <a href="tel:+919673185571" className="mt-4 block font-display text-3xl text-gold">+91 96 7318 5571</a>
              <a href="https://wa.me/919673185571" className="mt-3 inline-flex items-center gap-2 px-6 py-3 border border-gold text-xs tracking-[0.3em] uppercase hover:bg-gold hover:text-primary transition-colors">
                WhatsApp Us
              </a>
            </div>
            <div>
              <div className="eyebrow">Hours</div>
              <p className="mt-4 text-muted-foreground">Reception open 24 × 7</p>
            </div>
          </aside>
        </div>
      </section>
