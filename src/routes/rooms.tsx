          ...
          {rooms.map((r, i) => (
            <article key={r.name} className={`grid lg:grid-cols-2 gap-12 items-center ${i % 2 ? "lg:[&>div:first-child]:order-2" : ""}`}>
              ...
              <div className="lg:px-8">
                ...
                <h2 className="mt-4 font-display text-4xl md:text-5xl leading-tight">{r.name}</h2>
                <div className="mt-6 gold-line" />
                <p className="mt-6 text-lg leading-relaxed text-muted-foreground">{r.desc}</p>
                ...
                <a href="tel:+919673185571" className="mt-10 inline-block px-8 py-3.5 border border-gold text-xs tracking-[0.3em] uppercase hover:bg-gold hover:text-primary transition-colors">
                  Reserve
                </a>
              </div>
            </article>
